...
kesalahan atau gangguan. Pada model termal satelit LAPAN-A3 yang ditampilkan
pada Persamaan \ref{eq:lineq}, $Y$ mewakili laju perubahan suhu
\textit{node-node} satelit, $X$ parameter dan hasil perkalian parameter satelit
yang berubah terhadap waktu, $w$ koefisien suku-suku pada persamaan termal, dan
$\varepsilon$ kesalahan pada hasil pengukuran parameter.  

Model regresi linear \textit{machine learning} termasuk dalam
\textit{supervised learning} dan akan menghitung koefisien suku-suku pada
Persamaan \ref{eq:lineq} yang belum diketahui berdasarkan dataset masukan model
\textit{machine learning}. Data masukan yang diberikan ke model adalah
parameter \textit{node} yang berubah terhadap waktu sesuai Persamaan
\ref{eq:lineq} sedangkan data keluaran yang diinginkan adalah laju perubahan
suhu \textit{node}. Hasil perhitungan koefisien suku-suku pada persamaan termal
